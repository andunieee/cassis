...
let of_json_to_json label input =
  let fn _ =
    check string label input
      (Operation.of_json_string input |> Operation.to_json_string)
  in
  fn
...
let () =
  run "operation"
    [
      ( "json",
        [
          test_case "trust of json to json" `Quick
            (of_json_to_json "operation"
          ...
          test_case "send of json to json" `Quick
            (of_json_to_json "send"
        ...
        [
          test_case "private key to public key" `Quick (fun _ ->
              check string "pubkey"
                (Bip340.load_secret
                   (`Hex
                      "e09a4ae9d179c85c1842ad53e19cad764f471ab8072b7cb7f8d58a76aa62e0aa"
                  |> Hex.to_bytes)
                |> Bip340.public_key |> Hex.of_bytes |> Hex.show)
