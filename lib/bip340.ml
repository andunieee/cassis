...
let secp256k1_keypair_xonly_pub =
  foreign "secp256k1_keypair_xonly_pub"
    (secp256k1_context @-> ptr secp256k1_xonly_pubkey @-> ptr int
   @-> ptr secp256k1_keypair @-> returning int)

let secp256k1_xonly_pubkey_serialize =
  foreign "secp256k1_xonly_pubkey_serialize"
    (secp256k1_context @-> ptr char @-> ptr secp256k1_xonly_pubkey
   @-> returning int)
